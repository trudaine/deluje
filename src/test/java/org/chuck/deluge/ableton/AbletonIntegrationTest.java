...
public class AbletonIntegrationTest {
  ...
  public void testImportAbletonSet() throws Exception {
    ...
    org.chuck.deluge.model.ProjectModel project = new org.chuck.deluge.model.ProjectModel();
    ...
    // Assert tracks
    java.util.List<org.chuck.deluge.model.TrackModel> tracks = project.getTracks();
    ...
    for (org.chuck.deluge.model.TrackModel t : tracks) {
      ...
      if (t instanceof org.chuck.deluge.model.SynthTrackModel stm) {
        System.out.println("    Clip Count: " + stm.getClips().size());
        for (org.chuck.deluge.model.ClipModel cm : stm.getClips()) {
          System.out.println(
              "      Clip Name: \""
                  + cm.getName()
                  + "\", steps: "
          ...
          for (int r = 0; r < cm.getRowCount(); r++) {
            java.util.List<org.chuck.deluge.model.HighResNote> notes = cm.getRawNoteEvents(r);
            if (notes != null && !notes.isEmpty()) {
              ...
              System.out.println(
                  "        Row "
                      ...
                      + notes.size()
                      + " high-res notes:");
              for (org.chuck.deluge.model.HighResNote n : notes) {
                System.out.println(
                    "          Note: tickPos="
                        + n.getTickPos()
                        + ", tickLen="
      ...
      } else if (t instanceof org.chuck.deluge.model.KitTrackModel ktm) {
        System.out.println("    Drum Channel Count: " + ktm.getDrums().size());
        for (org.chuck.deluge.model.Drum d : ktm.getDrums()) {
        ...
        for (org.chuck.deluge.model.ClipModel cm : ktm.getClips()) {
          System.out.println(
              "      Clip Name: \""
                  + cm.getName()
                  + "\", steps: "
  ...
  public void testRoundTripExportParity() throws Exception {
    ...
    org.chuck.deluge.model.ProjectModel project1 = new org.chuck.deluge.model.ProjectModel();
    ...
    org.chuck.deluge.model.ProjectModel project2 = new org.chuck.deluge.model.ProjectModel();
    ...
    for (int i = 0; i < project1.getTracks().size(); i++) {
      org.chuck.deluge.model.TrackModel t1 = project1.getTracks().get(i);
      org.chuck.deluge.model.TrackModel t2 = project2.getTracks().get(i);

      assertEquals(t1.getName(), t2.getName(), "Track names must match");
      assertEquals(t1.getType(), t2.getType(), "Track types must match");

      if (t1 instanceof org.chuck.deluge.model.SynthTrackModel stm1) {
        org.chuck.deluge.model.SynthTrackModel stm2 = (org.chuck.deluge.model.SynthTrackModel) t2;
        assertEquals(
            stm1.getClips().size(), stm2.getClips().size(), "Synth track clip count must match");

        for (int c = 0; c < stm1.getClips().size(); c++) {
          org.chuck.deluge.model.ClipModel cm1 = stm1.getClips().get(c);
          org.chuck.deluge.model.ClipModel cm2 = stm2.getClips().get(c);
      ...
      } else if (t1 instanceof org.chuck.deluge.model.KitTrackModel ktm1) {
        org.chuck.deluge.model.KitTrackModel ktm2 = (org.chuck.deluge.model.KitTrackModel) t2;
        assertEquals(
            ktm1.getDrums().size(),
            ktm2.getDrums().size(),
        ...
        for (int d = 0; d < ktm1.getDrums().size(); d++) {
          org.chuck.deluge.model.Drum d1 = ktm1.getDrums().get(d);
          org.chuck.deluge.model.Drum d2 = ktm2.getDrums().get(d);

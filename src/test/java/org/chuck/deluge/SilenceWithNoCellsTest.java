...
/**
 * Verifies the engine is silent when PLAY=1 but no pattern cells are active.
 * Reproduces the "continuous sound on empty pattern" bug.
 */
public class SilenceWithNoCellsTest {
    ...
    void classicEngine_shouldBeSilentWithNoCells() throws Exception {
        ...
        System.out.printf("[DIAG] g_synth_bus = %s%n", synthBusObj == null ? "null" : synthBusObj.getClass().getSimpleName());
        if (synthBusObj instanceof org.chuck.audio.ChuckUGen synthBus) {
            ...
            for (int di = 0; di < synSrcs.size(); di++) {
                ...
                System.out.printf("[DIAG]   src[%d] = %s%n", di, s == null ? "null" : s.getClass().getSimpleName());
                if (s != null) {
                    for (org.chuck.audio.ChuckUGen ss : s.getSources()) {
                        System.out.printf("[DIAG]     -> %s%n", ss == null ? "null" : ss.getClass().getSimpleName());
                    }
                }
            }
        }
        ...
        assertEquals(0.0, rms, 1e-6,
            ...
            "Non-zero RMS=" + rms + " — check DEBUG_AUDIO output above for the source.");
    }

...
/**
 * Robust End-to-End Test for Deluge Audio Output.
 * Focuses on peak-meter analysis to prove actual signal generation.
 */
public class DelugeAudioE2ETest {
    ...
    public void test808KitProducesSound() throws Exception {
        ...
        System.out.printf("DIAGNOSTIC: Peak L: %.6f, Peak R: %.6f, Step Advanced: %s\n", peakL, peakR, stepAdvanced);

        vm.shutdown();
        
        assertTrue(stepAdvanced, "Engine playhead did not advance.");
        assertTrue(peakL > 0.01f || peakR > 0.01f, 
            String.format("Audio signal too weak. Peaks: L=%.6f, R=%.6f. Expecting > 0.01", peakL, peakR));
    }
}

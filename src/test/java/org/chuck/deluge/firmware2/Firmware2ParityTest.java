...
/**
 * Ports of the Deluge firmware's own unit tests ({@code tests/unit/lfo_tests.cpp},
 * {@code tests/unit/value_scaling_tests.cpp}) to verify firmware2 produces
 * bit-identical results. Every expected value comes directly from the C++ test source.
 */
public class Firmware2ParityTest {
  ...
  public void getTriangleEdgeCases() {
    ...
    for (int p : new int[]{0, 1, 10, 1000, 1073741823, 1073741824, 1073741825, 2000000000, -1, -2147483648}) {
      int t = Functions.getTriangle(p);
      assertTrue(t >= -2147483648 && t <= 2147483647,
  ...
  public void lfoTrianglePhaseAccumulation() {
    ...
    lfo.render(10, Lfo.LfoType.TRIANGLE, 100);
    assertEquals(-2147482648, lfo.phase, "phase advances from INT32_MIN by 1000"); // unsigned: 0x80000000+1000=0x800003E8
  }
  ...
  public void standardMenuItemValues() {
    assertEquals(-2147483648,
        Functions.getParamFromUserValue(Param.LOCAL_VOLUME, 0));
    assertEquals(-23,
        Functions.getParamFromUserValue(Param.LOCAL_VOLUME, 25));
    assertEquals(2147483602,
  ...
  public void lookupReleaseRateNeutral() {
    ...
    assertTrue(result > 4000000 && result < 5000000,
  ...
  public void volumeCurveAtOffKnob() {
    int result = Functions.getFinalParameterValueVolume(134217728,

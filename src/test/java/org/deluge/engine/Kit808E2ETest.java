...
class Kit808E2ETest {
  ...
  private static org.deluge.playback.Sample makeSample(float freqHz) {
    int sr = 44100;
    int n = sr / 10; // 100ms
    ...
    var s = new org.deluge.playback.Sample();
    s.data = data;
    s.numChannels = 1;
    s.sampleRate = sr;
    return s;
  ...
  private static void applySampleToDrum(
      FirmwareKit kit, int drumIdx, org.deluge.playback.Sample ms) {
  ...
  void distinctSamplesProduceDistinctKitDrumAudio() {
    ...
    var samples = new org.deluge.playback.Sample[4];

...
public class FirmwareFilterModeTest {
  ...
  private static FirmwareSound buildSaw(float hpfHz) {
    ...
    org.deluge.modulation.automation.AutoParam ap =
        sound.paramManager.getAutomatedParam(org.deluge.firmware2.Param.LOCAL_HPF_FREQ);
    if (ap != null) {
      ap.currentValue = sound.paramNeutralValues[org.deluge.firmware2.Param.LOCAL_HPF_FREQ];
    }
    return sound;
  }

  private static void syncAuto(FirmwareSound s, int paramId) {
    org.deluge.modulation.automation.AutoParam ap = s.paramManager.getAutomatedParam(paramId);

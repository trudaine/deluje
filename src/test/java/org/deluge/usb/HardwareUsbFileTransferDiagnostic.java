...
/**
 * Diagnostic tool to test the USB file listing and downloading features. Connects to the physical
 * Deluge via CDC serial, lists the "/SONGS" folder, and attempts to download the first XML song
 * file found.
 */
public class HardwareUsbFileTransferDiagnostic {
  ...
  public static void main(String[] args) {
    ...
    service.addFileListener(
        new DelugeUsbSyncService.UsbFileListener() {
          @Override
          ...
          public void onFileChunkReceived(int chunkIndex, boolean isEof, byte[] data) {
            System.out.printf(
                "[USB] Received file chunk: %d (size: %d, isEof: %b)%n",
                chunkIndex, data.length, isEof);
            try {
    ...
    try {
      ...
      if (listLatch.await(10, TimeUnit.SECONDS)) {
        ...
        System.err.println(
            "Timeout waiting for device connection. Is the Deluge connected via USB and turned on?");
      }

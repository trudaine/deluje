...
public class PerSoundDelayTimingTest {
  ...
  /**
   * Guards against the delay tail being cut when the source voice dies before the echo:
   * TestDelayTailSurvival is TestDelayFidelity with envelope1 sustain forced to 0x80000000 (no
   * sustain), so the 124ms note fully decays and the voice is unassigned within ~0.3s — long before
   * the 1.0s syncLevel-4 echo. If Sound stopped rendering as soon as its voices die (instead of
   * gating on Delay.repeatsUntilAbandon, C sound.cpp:2164-2166), the echo would never be rendered
   * at all.
   */
  @Test
  void delayEchoSurvivesVoiceDeathBeforeEchoTime() throws Exception {
    File songFile = new File("src/main/resources/SONGS/TestDelayTailSurvival.xml");
    if (!songFile.exists()) {
      songFile = new File("../deluge/src/main/resources/SONGS/TestDelayTailSurvival.xml");
    }
    ProjectModel pm = DelugeXmlParser.parseSong(songFile);
    ProjectModel song = FirmwareFactory.createSong(pm);
    FirmwareAudioEngine eng = new FirmwareAudioEngine();
    ...
    // The dry voice must actually be dead (near silence) well before the echo lands, or this
    // test wouldn't be exercising the "voice died first" scenario at all.
    int quietStart = (int) (0.5 / (win / (double) sr));
    int quietEnd = (int) (0.85 / (win / (double) sr));
    float quietPeak = 0;
    for (int i = quietStart; i < quietEnd && i < ne; i++) quietPeak = Math.max(quietPeak, env[i]);
    assertTrue(
        quietPeak < 0.04 * peak,
        "expected the dry voice to have fully decayed between 0.5s-0.85s (voice-death window),"
            + " got relative level "
            + (quietPeak / peak));

    // Despite the voice being dead, the syncLevel-4 (~1.0s) echo must still render.
    java.util.List<Double> onsets = new java.util.ArrayList<>();
    double lastT = -1;
    for (int i = 0; i < ne; i++) {
      double t = i * win / (double) sr;
    ...
    assertTrue(onsets.size() >= 2, "expected a dry hit + a surviving echo; got " + onsets);
    double firstEcho = onsets.get(1) - onsets.get(0);
    assertEquals(
        1.0,
        firstEcho,
        0.1,
        "delay tail must survive voice death and still echo ~1.0s after the dry hit");
  }
}

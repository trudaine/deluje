...
public class SongGridPanelSoloTest {
  ...
  public void testPlayModeStatusColours() throws Exception {
    ...
    assertEquals(
        new Color(245, 190, 0),
        mutePad.getBackground(),
        "Status pad must reflect active clip play mode (ONCE)");
    ...
    assertEquals(
        new Color(0, 255, 6),
        mutePad.getBackground(),
        "Status pad must reflect active clip play mode (NORMAL)");
  ...
  public void testCrossScreenWrapEditing() throws Exception {
    ...
    java.awt.event.MouseWheelEvent wheelEv =
    ...
    StepPropertiesEditor.applyStepProperties(
        panel.clipController, 67, 4, 0.5, 0, 0.0, 0.5, 1.0, 0.0);
    assertEquals(0.5f, clip.getStep(0, 4).velocity(), 1e-3f);
    assertEquals(0.5f, clip.getStep(0, 4).probability(), 1e-3f);
    assertEquals(
        0.5f, clip.getStep(0, 20).velocity(), 1e-3f, "Context menu velocity edits must wrap");
    assertEquals(
        0.5f, clip.getStep(0, 20).probability(), 1e-3f, "Context menu probability edits must wrap");

...
public class PhysicalHardwareFidelityTest {
    ...
    try {
      ...
      for (int lag = -5; lag <= 5; lag++) {
        if (swZeroIdx + lag >= 0 && swZeroIdx + lag + windowSize <= noDcSw.length) {
          ...
          double c = org.deluge.AudioAnalyzer.correlation(hwWindow, swWindow);
          if (c > correlation) {
            correlation = c;
          }
        }
  ...
  public void testFilteredLPFParity() throws Exception {
    ...
    synthModel.setFilterMode(org.deluge.model.FilterMode.LADDER_24);
  ...
  private void assertWaveShapeFidelity(
    ...
    double hwPitchVal =
        org.deluge.AudioAnalyzer.estimateFrequency(hwWindow, 44100, minPitchFreq, maxPitchFreq);
    double swPitchVal =
        org.deluge.AudioAnalyzer.estimateFrequency(swWindow, 44100, minPitchFreq, maxPitchFreq);
    System.out.printf(
  ...
  private void assertSpectralFidelity(
    ...
    double corr = org.deluge.AudioAnalyzer.correlation(a, b);
    ...
    try {
      ...
      float[] sw =
          renderXmlTrackPreset(
              ...
              triggerBlock + 1000,
              72,
              java.util.Map.of(org.deluge.firmware2.Param.LOCAL_VOLUME, 134217728));
      int hwStart = findPositiveZeroCrossing(hw, 10000);
      int swStart = findPositiveZeroCrossing(sw, 12800);
      System.out.println("=================================");
  ...
  public void testHooverBassRecordingParity() throws Exception {
    ...
    try {
      ...
      double swPitch = org.deluge.AudioAnalyzer.estimateFrequency(swWin, 44100, 50.0, 100.0);
      System.out.printf(
  ...
  public void testSynthDualModRecordingParity() throws Exception {
    ...
    double hwPitch = org.deluge.AudioAnalyzer.estimateFrequency(hwWin, 44100, 400.0, 1000.0);
    double swPitch = org.deluge.AudioAnalyzer.estimateFrequency(swWin, 44100, 400.0, 1000.0);
  ...
  private double getWindowCorrelation(
    ...
    for (int lag = -350; lag <= 350; lag++) {
      ...
      double corr = Math.abs(org.deluge.AudioAnalyzer.correlation(hWin, sWin));
      if (corr > maxCorr) {
        maxCorr = corr;
      }
    }

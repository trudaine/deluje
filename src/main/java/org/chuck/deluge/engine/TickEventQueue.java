 ...
 *
 * <p>The clock thread writes scheduled note-on/off events; the audio render
 * loop reads them before each buffer render, dispatching voice triggers at
 * precise sample positions within the buffer.
 *
 * <p>This is a single-producer (clock thread), single-consumer (audio thread)
...
public class TickEventQueue {
    ...
    /**
     * Drain all pending events from the queue (called from audio thread — single consumer).

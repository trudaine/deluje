...
/**
 * State Variable Filter (SVF) with ZDF (zero-delay feedback) topology, matching the chuck-core
 * {@code SVFilter} class in compute() behavior only.
 *
 * <p>Implements the same ZDF loop with tanh saturation on bandpass state. Morph control: 0=LP,
 * 0.5=BP, 1.0=HP. The filter state (ic1eq/ic2eq) persists across calls, matching the DSL behavior
 * where filter state is only reset during initialization, not per voice trigger.
 */
public class NativeSVFilter {
   ...
   *
   * <p>Matches the DSL's SVFilter.compute() — double-sampled ZDF with tanh saturation on bandpass,
   * no gain multiplication (gain=1.0 by default in DSL).
   */

...
/**
 * Wraps the native DX7 JNI library (deluge_dsp_native.dll) for use in {@code NativeJavaSequencer} —
 * drop-in replacement for {@link NativeDx7Voice} that uses native C++ code instead of the Java
 * Dx7Engine.
 *
 * <p>Includes the DSP chain matching the DSL engine exactly: FfmDx7Engine (C++ DX7 engine) ->
 * SVFilter -> HPF -> outputGain.
 *
 * <p>The native engine processes 132-sample blocks internally and serves per-sample output via
 * buffering, making it far more efficient than the Java Dx7Engine which does per-sample computation
 * at ~50x CPU cost.
 */
public class NativeDx7VoiceNative {
  ...
  /**
   * Global native engine initializer — call once before creating any voices. Safe to call multiple
   * times (no-op after first).
   */
  ...
  /**
   * Force-destroy the native voice handle. Called on clearTrack() to release native resources.
   * After this, trigger() will create a new handle.
   */
   ...
   *
   * <p>Signal chain: FfmDx7Engine.tick() -> SVFilter -> HPF * outputGain
   *
   * <p>NOTE: No ADSR in this chain. The Dx7Engine's per-operator logarithmic envelopes (dexed/msfa)
   * handle all amplitude shaping. outputGain applies track-level volume.
   */
  public float tick() {
  ...
  /**
   * Returns true while this voice is producing sound. The native voice may still be active
   * (envelope releasing) even after note-off.
   */
   ...
   * prevents orphaned voices from contributing to the mix.
   */

 ...
 *
 * <p>Includes DSP chain matching the DSL engine exactly: Dx7Engine -> SVFilter -> HPF -> env(gain).
 * This ensures the spectral content and amplitude match the DSL engine.
 *
 * <p>The DSL chain routes Dx7Engine through SVFilter -> HPF -> env(gain) -> Pan. The SVFilter (ZDF
 * topology with tanh saturation) significantly shapes the Dx7Engine's high-amplitude output (up to
 * +/-2.3), attenuating pathologically loud harmonics. The HPF removes subsonic content below 20 Hz.
 * These filters are the primary reason for the 4x amplitude difference between the raw Dx7Engine
 * output and the DSL engine's output.
 *
 * <p>Filter state (SVFilter ic1eq/ic2eq, HPF x1/x2/y1/y2) persists across note triggers, matching
 * the DSL behavior where filter state is only reset during initialization, not per voice trigger.
 */
public class NativeDx7Voice {
   ...
   *
   * <p>Note: ADSR params (attack/decay/sustain/release) are accepted for API compatibility but
   * ignored — the Dx7Engine's per-operator envelopes handle all amplitude shaping. Only filter
   * params and gain are applied.
   *
   ...
   *
   * <p>Signal chain: Dx7Engine.tick() -> SVFilter.compute() -> HPF.compute() * outputGain
   *
   * <p>NOTE: No ADSR in this chain. The Dx7Engine's per-operator logarithmic envelopes (dexed/msfa)
   * handle all amplitude shaping for attack, decay, sustain, and release. The ADSR multiply in the
   * DSL chain (env[i][0] DelugeAdsr) is also bypassed for DX7 tracks. outputGain applies
   * track-level volume without envelope shaping.
   */
  public float tick() {
  ...
  /**
   * Returns true while this voice is producing sound. The voice is active only while the Dx7Engine
   * has operator activity.
   */
  ...
  /**
   * Clear track assignment to stop rendering in the render loop. Called when the voice is recycled
   * by the allocator after release completes.
   */
  ...
  /**
   * Returns true only for voices that are both active AND currently assigned to a track. This
   * prevents orphaned voices (released but still rendering via isActive()) from contributing to the
   * mix.
   */

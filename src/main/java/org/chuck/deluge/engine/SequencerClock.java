 ...
 *
 * <p>Replaces the {@link java.util.concurrent.ScheduledExecutorService}-based approach
 * with a tight spin-wait loop that delivers step ticks with swing-aware timing,
 * stutter support, and bar-boundary clip queue processing.
 *
 * <p>Matching the behavior of {@code DelugeEngineDSL.clock_shred()}:
 * <ul>
 *   <li>Swing per even/odd step (same formula: {@code 1.0 ± (swing - 0.5) * 0.4})</li>
 *   <li>Stutter mode: repeat current step at sub-division rate when {@code G_STUTTER_ON}</li>
 *   <li>Transport-aware: stops advancing when {@code G_PLAY == 0}</li>
 *   <li>Bar-boundary clip queue processing at {@code step % 16 == 0}</li>
 * </ul>
 */
public class SequencerClock implements Runnable {
    ...
    public interface TickListener {
        /**
         * Called on every step tick. Runs on the clock thread — implementations
         * must be fast (ideally just writing to shared volatile fields).
         *

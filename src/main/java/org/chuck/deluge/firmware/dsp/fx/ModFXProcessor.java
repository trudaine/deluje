...
public class ModFXProcessor {
  ...
  public void resetMemory() {
    ...
    for (StereoSample s : allpassMemory) {
      s.l = 0;
      s.r = 0;
    }
    phaserMemory.l = 0;
        ...
        if (modFXType == ModFXType.WARBLE) {
          lfo2 =
              modFXLFOStereo.render(
                  1, waveType, multiply_32x32_rshift32(modFXRate, (int) (0.97 * ONE)) << 1);
        } else {
          lfo2 = -lfo1;
        }
  ...
  private void processOneModFXSample(
    ...
    int outL =
        multiply_32x32_rshift32_rounded(modFXBuffer[pos1 & kModFXBufferIndexMask].l, strength1)
            + multiply_32x32_rshift32_rounded(
                modFXBuffer[(pos1 - 1) & kModFXBufferIndexMask].l, strength2);

    // Right channel
    int delayTimeR =
        (type == ModFXType.DIMENSION || type == ModFXType.WARBLE)
            ? multiply_32x32_rshift32(lfo2, delayDepth) + delayOffset
            : delayTime;
    ...
    int outR =
        multiply_32x32_rshift32_rounded(modFXBuffer[pos1R & kModFXBufferIndexMask].r, strength1R)
            + multiply_32x32_rshift32_rounded(
                modFXBuffer[(pos1R - 1) & kModFXBufferIndexMask].r, strength2R);
  ...
  private void processOnePhaserSample(StereoSample s, int depth, int feedback, int lfo) {
    int _a1 =
        1073741824
            - multiply_32x32_rshift32_rounded((int) (((long) lfo + 2147483648L) >> 1), depth);

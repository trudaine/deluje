...
/**
 * Port of FFTConfigManager from the firmware. Manages FFT configurations for spectral processing
 * with bit-accurate fixed-point math.
 */
public class FFTConfigManager {
  public static class FFTConfig {
    ...
    /**
     * Performs a bit-accurate fixed-point forward FFT. Replicates the behavior of the hardware's
     * NE10_INT32 R2C FFT logic.
     */
      ...
      for (int stage = 1; stage <= magnitude; stage++) {
        ...
        for (int k = 0; k < n; k += m) {
          ...
          for (int i = 0; i < halfM; i++) {
            ...
            int tr =
                (int) (((long) real[k + i + halfM] * cos - (long) imag[k + i + halfM] * sin) >> 31);
            int ti =
                (int) (((long) real[k + i + halfM] * sin + (long) imag[k + i + halfM] * cos) >> 31);

 ...
 *
 * <p>Every numeric type in this package maps to its C++ counterpart with
 * <strong>identical bit-width and two's complement representation</strong>.
 * Where the C++ type is unsigned and Java has no direct equivalent, the
 * standard Java idiom of <em>store as signed, widen with mask</em> preserves
 * bit-accuracy.</p>
 *
 ...
 * <p>A value is <strong>bit-accurate</strong> iff its 16/32/64-bit
 * memory representation matches the C++ original at every index.
 * Widening with a mask ({@code &amp; 0xFFFF}) is acceptable because it
 * extracts the stored bits without altering them — it merely changes the
 * interpretation on read. Direct arithmetic on a {@code short} value
 * that was stored as {@code uint16_t} (without first masking) is
 * <em>not</em> bit-accurate.</p>
 *
 * <h3>Q31 fixed-point convention</h3>
 * <p>The Deluge firmware uses Q31 (1.31 signed fixed-point) extensively.
 * Values range from {@code -2147483648} ({@code -1.0}) to
 * {@code 2147483647} ({@code ≈1.0 - 2⁻³¹}). Multiplication is performed
 * via {@code multiply_32x32_rshift32()} which multiplies two Q31 values
 * and right-shifts by 32 to keep the result in Q31. This is identical in
 * C++ and Java since both use 32-bit two's complement for {@code int}.</p>
 *

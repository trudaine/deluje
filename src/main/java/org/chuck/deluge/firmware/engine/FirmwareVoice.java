...
public class FirmwareVoice {
  ...
  public boolean render(int[] buffer, int numSamples, int phaseIncrementA, int phaseIncrementB) {
    ...
    // 1. Process Envelopes
    // ── High-Fidelity ADSR ──
    // Use smaller increments for sane timings

    // For drums, sustain MUST be 0
    int sus = (sound.isDrum) ? 0 : (int) (ONE * 0.7);

    int env0 = envelopes[0].render(numSamples, 20000, 400, sus, 400, LookupTables.decayTableSmall8);
    ...
    for (int i = 0; i < numSamples; i++) {
      ...
      // Overwrite buffer with gained signal (don't add to itself, that causes distortion and
      // infinite sustain!)
      buffer[i] = wet;
    }
    return true;
    ...
    for (int s = 0; s < 2; s++) {
      ...
      if (type == OscType.SAMPLE) {
        ...
        Oscillator.renderOsc(
            type, vol, buffer, 0, numSamples, adjPInc, pw, phase, true, 0, false, 0, 0, 0);

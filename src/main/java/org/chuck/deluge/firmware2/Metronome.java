 ...
 * square wave (high then low) that decays after ~1000 samples. The song's post-FX volume (C reads
 * it from {@code currentSong->paramManager}) is injected into {@link #render} as a seam.
 */
public class Metronome {
  ...
  public void setVolume(int linearParam) {
    metronomeVolume =
        (int)
            ((Math.exp((double) ((float) linearParam / 200.0f)) - 1.0)
                * (double) (float) (1 << 27));
  }

  /**
   * C: metronome.cpp:38-67. {@code volumePostFX} is the seam the C derives from the song (or {@code
   * ONE_Q31} when there is no song).
   */
  public void render(int[][] buffer, int numSamples, int volumePostFX) {
    ...
    for (int i = 0; i < numSamples; i++) {
      int value =
          (Integer.compareUnsigned(phase, ONE_Q31) <= 0) ? high : -high; // C:58 (phase <= ONE_Q31)

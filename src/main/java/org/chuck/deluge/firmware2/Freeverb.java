 ...
 * <p>C sources:
 * <ul>
 * <li>{@code dsp/reverb/freeverb/freeverb.cpp} (105 lines) + {@code freeverb.hpp} (165)</li>
 * <li>{@code dsp/reverb/freeverb/comb.hpp} (76 lines)</li>
 * <li>{@code dsp/reverb/freeverb/allpass.hpp} (62 lines)</li>
 * <li>{@code dsp/reverb/freeverb/tuning.h} (72 lines)</li>
 * <li>{@code dsp/reverb/base.hpp} (63 lines)</li>
 * </ul>
 *
 * <p>Freeverb is a Schroeder-Moorer reverb: 8 parallel comb filters → 4 series allpass filters.
 * Stereo width, room size, damping, and wet/dry mix are adjustable. Input is summed to mono
 * before processing.</p>
 */
public class Freeverb {
  ...
  public static class Comb {
    ...
    public int process(int input) {
      ...
      filterstore = (Functions.multiply_32x32_rshift32_rounded(output, damp2)
                   + Functions.multiply_32x32_rshift32_rounded(filterstore, damp1)) << 1;

      // C:58
      buffer[bufidx] = input + (Functions.multiply_32x32_rshift32_rounded(filterstore, feedback) << 1);
  ...
  /**
   * C: freeverb.hpp:98-109 — process a buffer of mono input samples.
   * Output is accumulated (added) into the outputLR array of [l, r] per sample.
   * Input array is modified in-place (HPF applied).
   */

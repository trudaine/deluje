...
public class SincInterpolator {
  ...
  // C: interpolate.h:15-16 — kInterpolationMaxNumSamples = 16 (2^4)
  static final int K_INTERPOLATION_MAX_NUM_SAMPLES = 16;
  static final int K_INTERPOLATION_MAX_NUM_SAMPLES_MAGNITUDE = 4;
  static final int NUM_BITS_IN_TABLE_SIZE = 8;

  // C: interpolate.h:41-42 — state buffers (circular history)
  final short[] bufferL = new short[K_INTERPOLATION_MAX_NUM_SAMPLES];
  final short[] bufferR = new short[K_INTERPOLATION_MAX_NUM_SAMPLES];

  // ── pushL / pushR (interpolate.h:16-21, 24-29) ──

  /** C: interpolate.h:16-22 — push sample into left history */
  public void pushL(short value) {
    for (int i = K_INTERPOLATION_MAX_NUM_SAMPLES - 1; i >= 1; i--) {
      bufferL[i] = bufferL[i - 1];
    }
    bufferL[0] = value;
  }

  /** C: interpolate.h:24-29 — push sample into right history */
  public void pushR(short value) {
    for (int i = K_INTERPOLATION_MAX_NUM_SAMPLES - 1; i >= 1; i--) {
  ...
  // ── interpolate (interpolate.cpp:9-68) ──

  /**
   * C: interpolate.cpp:9-68 — scalar port of NEON sinc convolution.
   *
   * @param channels 1=mono, 2=stereo
   * @param whichKernel kernel table index (0-6)
  ...
  public int[] interpolate(int channels, int whichKernel, int oscPos) {
    ...
    // C:44-46 — MultiplyAddLong(kernel, buffer_l)
    // NEON: 8-lane multiply-accumulate of int16 kernel × int16 samples → int32 sum
    int sumL = 0;
    for (int i = 0; i < K_INTERPOLATION_MAX_NUM_SAMPLES; i++) {

...
/**
 * Faithful line-by-line port of the Deluge {@code sine_osc.h} SineOsc class.
 * The firmware uses ARM SIMD ({@code Argon<uint32_t>}) vectors; this Java port
 * uses the equivalent scalar operations, matching the firmware sample-by-sample.
 *
 * <p>The sine table is an interleaved array of 512 {@code int16_t} entries:
 * even indices hold the sine value, odd indices hold the difference (slope)
 * to the next table entry. {@code doFMNew} performs a single linear-interpolated
 * sine lookup with an external phase offset (used for FM/phase modulation).
 */
public final class SineOsc {
  ...
  /**
   * Interleaved sine-difference table matching {@code sineWaveDiff[512]} from the firmware.
   * Even indices: sine value (int16_t). Odd indices: difference to next entry (int16_t).
   * Total: 512 entries (256 sine values + 256 diffs).
   */
  ...
  /**
   * Port of {@code SineOsc::doFMNew(uint32_t carrierPhase, uint32_t phaseShift)}.
   * A single linearly-interpolated sine lookup at phase {@code (carrierPhase >> 8) + phaseShift}.
   * The 24-bit effective phase wraps at 2^24 (16777216) due to the table size.
   *
  ...
  public static int doFMNew(int carrierPhase, int phaseShift) {
    ...
    int readValue = (SINE_WAVE_DIFF[readOffset] & 0xFFFF)
        | ((SINE_WAVE_DIFF[readOffset + 1] & 0xFFFF) << 16);
  ...
  /**
   * Scalar equivalent of {@code render()}: four-sample sine via linear interpolation.
   * Matches the SIMD vector path (@code Argon<uint32_t>}) sample-by-sample.
   * Each of the 4 samples uses the phase increment to advance the phase.
   *
  ...
  /**
   * Vector-equivalent doFM: four carriers with four phase shifts.
   * Port of {@code doFMVector(phaseVector, phaseShift)}.
   *

 ...
 * and {@link #render} (hop-shortening, percussiveness-cut, two-head crossfade mix). Seams:
 * {@code LiveInputBuffer} + {@code audioSampleTimer} + {@code inputBlock} are injected (the C
 * reads the hardware via {@code AudioEngine}).
 */
public class LivePitchShifter {
  ...
  /**
   * C: live_pitch_shifter.cpp:71-306 — the main render loop. INPUT_ENABLE_REPITCHED_BUFFER is 0, so the
   * repitched-buffer paths are omitted. Seams: {@code liveInputBuffer} (injected; the C reads
   * {@code AudioEngine::getOrCreateLiveInputBuffer}), {@code audioSampleTimer} (the C reads the hardware
   * timer; injected as the {@code currentTime} parameter to {@code giveInput}).
   */
  public void render(int[] outputBuffer, int numSamples, int phaseIncrement, int amplitude, int amplitudeIncrement,
      int interpolationBufferSize, LiveInputBuffer liveInputBuffer, int audioSampleTimer, int[] inputBlock) {
    liveInputBuffer.giveInput(inputBlock, numSamples, audioSampleTimer, inputType); // C:80 (input via seam)
    ...
    while (true) {
      ...
      if (!justDidHop && phaseIncrement > K_MAX_SAMPLE_VALUE) {
        int maxPlayableSamplesNewer =
            playHeads[TimeStretcher.PLAY_HEAD_NEWER].getEstimatedPlaytimeRemaining(liveInputBuffer, phaseIncrement); // C:129-135
        ...
        if (samplesTilHopEnd + nextCrossfadeLength > maxPlayableSamplesNewer) { // C:137
          ...
          if (samplesTilHopEnd < 0) { // C:145
            ...
            int minCrossfadeIncrement =
                Integer.divideUnsigned(K_MAX_SAMPLE_VALUE - crossfadeProgress, samplesTilHopEnd) + 1; // C:157
        ...
        if (samplesTilHopEnd != 0 && olderPlayHeadIsCurrentlySounding()) { // C:165
          int maxPlayableSamplesOlder =
              playHeads[TimeStretcher.PLAY_HEAD_OLDER].getEstimatedPlaytimeRemaining(liveInputBuffer, phaseIncrement); // C:166-172
          if (maxPlayableSamplesOlder == 0) { // C:174
            crossfadeIncrement = K_MAX_SAMPLE_VALUE; // C:175
          } else {
            int minCrossfadeIncrement =
                Integer.divideUnsigned(K_MAX_SAMPLE_VALUE - crossfadeProgress, maxPlayableSamplesOlder) + 1; // C:178-179
      ...
      if (!justDidHop && !olderPlayHeadIsCurrentlySounding() && samplesTilHopEnd != 0
          && playHeads[TimeStretcher.PLAY_HEAD_NEWER].mode != LivePitchShifterPlayHead.PlayHeadMode.RAW_DIRECT) { // C:192-193

        int howFarBack =
            playHeads[TimeStretcher.PLAY_HEAD_NEWER].getNumRawSamplesBehindInput(liveInputBuffer, phaseIncrement); // C:195-196

        int newerPlayHeadPercPos =
            (liveInputBuffer.numRawSamplesProcessed - howFarBack - 1) >> LiveInputBuffer.K_PERC_BUFFER_REDUCTION_MAGNITUDE; // C:198-199

        int latestPercPosBefore = (numRawSamplesProcessedAtStart - 1) >> LiveInputBuffer.K_PERC_BUFFER_REDUCTION_MAGNITUDE; // C:201
        int latestPercPosNow = (liveInputBuffer.numRawSamplesProcessed - 1) >> LiveInputBuffer.K_PERC_BUFFER_REDUCTION_MAGNITUDE; // C:202
        ...
        if (latestPercPosNow != newerPlayHeadPercPos
          ...
          int percLatest = liveInputBuffer.percBuffer[latestPercPosNow & (LiveInputBuffer.K_INPUT_PERC_BUFFER_SIZE - 1)] & 0xFF; // C:208
          int percNewerPlayHead = liveInputBuffer.percBuffer[newerPlayHeadPercPos & (LiveInputBuffer.K_INPUT_PERC_BUFFER_SIZE - 1)] & 0xFF; // C:209
      ...
      if (olderPlayHeadAudibleHere) {
        ...
        int newerHopAmplitudeAfter = Functions.lshiftAndSaturateUnknown(crossfadeProgress, 7); // C:249
        int newerHopAmplitudeIncrement =
            (newerHopAmplitudeAfter - newerHopAmplitudeNow) / numSamplesThisTimestretchedRead; // C:251-252
        int hopAmplitudeChange = Functions.multiply_32x32_rshift32(amplitude, newerHopAmplitudeIncrement) << 1; // C:255
        newerAmplitudeIncrementNow = amplitudeIncrement + hopAmplitudeChange; // C:257
        newerSourceAmplitudeNow = Functions.multiply_32x32_rshift32(amplitude, newerHopAmplitudeNow) << 1; // C:258
        olderAmplitudeIncrementNow = amplitudeIncrement - hopAmplitudeChange; // C:260
        olderSourceAmplitudeNow = Functions.multiply_32x32_rshift32(amplitude, olderHopAmplitudeNow) << 1; // C:261
      } else {
      ...
      playHeads[TimeStretcher.PLAY_HEAD_NEWER].render(tmp, numSamplesThisTimestretchedRead, numChannels,
          phaseIncrement, newerSourceAmplitudeNow, newerAmplitudeIncrementNow,
          liveInputBuffer.rawBuffer, whichKernel, interpolationBufferSize);

      // C:282-291 — older play-head
      if (olderPlayHeadAudibleHere) {
        playHeads[TimeStretcher.PLAY_HEAD_OLDER].render(tmp, numSamplesThisTimestretchedRead, numChannels,
            phaseIncrement, olderSourceAmplitudeNow, olderAmplitudeIncrementNow,
            liveInputBuffer.rawBuffer, whichKernel, interpolationBufferSize);
      }
   ...
   * pitch-up, simpler placement for pitch-down, then the bidirectional crossfade-point search over the
   * ring buffer. {@code INPUT_ENABLE_REPITCHED_BUFFER == 0}, so the repitched-buffer paths are omitted.
   */
  private void hopEnd(int phaseIncrement, LiveInputBuffer liveInputBuffer, int numRawSamplesProcessedAtNowTime,
    ...
    // C:436-437
    int lengthPerMovingAverage = (int) (((phaseIncrement & 0xFFFFFFFFL) * TimeStretcher.K_MOVING_AVERAGE_LENGTH) >>> 24);
    lengthPerMovingAverage = Math.max(1, Math.min(TimeStretcher.K_MOVING_AVERAGE_LENGTH * 2, lengthPerMovingAverage));

    int crossfadeLengthSamplesSource = (int) (((thisCrossfadeLength & 0xFFFFFFFFL) * (phaseIncrement & 0xFFFFFFFFL)) >>> 24); // C:441

    int maxOffsetFromHead =
        (playHeads[TimeStretcher.PLAY_HEAD_OLDER].rawBufferReadPos + LiveInputBuffer.K_INPUT_RAW_BUFFER_SIZE
            - numRawSamplesProcessedLatest) & (LiveInputBuffer.K_INPUT_RAW_BUFFER_SIZE - 1); // C:445-446
    ...
    lengthPerMovingAverage = Math.min(lengthPerMovingAverage, averagesEndOffsetFromHead >> 1); // C:458-459

    int averagesStartOffsetFromHead =
        averagesEndOffsetFromHead - lengthPerMovingAverage * TimeStretcher.K_NUM_MOVING_AVERAGES; // C:461-462
    ...
    if (lengthPerMovingAverage != 0) {
      ...
      liveInputBuffer.getAveragesForCrossfade(oldHeadTotals, averagesStartPosOldHead, lengthPerMovingAverage, numChannels); // C:471-472
    }
    ...
    if (phaseIncrement > K_MAX_SAMPLE_VALUE) {
      minSearch += Functions.multiply_32x32_rshift32(
          minSearch, Functions.multiply_32x32_rshift32(Functions.getNoise(), randomElement << 8)) << 2; // C:497-498

      int backEdge = minSearch >> LiveInputBuffer.K_PERC_BUFFER_REDUCTION_MAGNITUDE; // C:501
      int howFarBackSearched = 0;
      int percPos = (numRawSamplesProcessedAtNowTime + LiveInputBuffer.K_PERC_BUFFER_REDUCTION_SIZE - 1)
      ...
      while (backEdge < (maxSearch >> LiveInputBuffer.K_PERC_BUFFER_REDUCTION_MAGNITUDE)) { // C:510
        while (howFarBackSearched < backEdge) { // C:512
          ...
          int percHere = liveInputBuffer.percBuffer[
              (percPos - howFarBackSearched) & (LiveInputBuffer.K_INPUT_PERC_BUFFER_SIZE - 1)] & 0xFF; // C:517-519
      ...
      samplesTilHopEnd =
          (int) (((howFarBack & 0xFFFFFFFFL) << 24) / ((phaseIncrement - K_MAX_SAMPLE_VALUE) & 0xFFFFFFFFL))
      ...
      int minDistanceBack = numRawSamplesProcessedAtNowTime - numRawSamplesProcessedLatest
          + averagesStartOffsetFromHead + lengthPerMovingAverage * TimeStretcher.K_NUM_MOVING_AVERAGES; // C:553-555
      howFarBack = Math.max(howFarBack, minDistanceBack); // C:556
      if (howFarBack > numRawSamplesProcessedAtNowTime) howFarBack = numRawSamplesProcessedAtNowTime; // C:558-560
      ...
      playHeads[TimeStretcher.PLAY_HEAD_NEWER].rawBufferReadPos =
          (numRawSamplesProcessedAtNowTime - howFarBack) & (LiveInputBuffer.K_INPUT_RAW_BUFFER_SIZE - 1); // C:573-574
    }
    ...
    // C:578-598 — pitch DOWN: simpler, uses maxHopLength
      ...
      if (lengthPerMovingAverage != 0) { // C:581
        averagesStartPosNewHead =
            (numRawSamplesProcessedLatest - lengthPerMovingAverage * TimeStretcher.K_NUM_MOVING_AVERAGES)
      ...
      playHeads[TimeStretcher.PLAY_HEAD_NEWER].rawBufferReadPos =
          (numRawSamplesProcessedLatest - lengthPerMovingAverage * TimeStretcher.K_NUM_MOVING_AVERAGES
              - averagesStartOffsetFromHead) & (LiveInputBuffer.K_INPUT_RAW_BUFFER_SIZE - 1); // C:593-597
    }

    int bestOffset = 0; // C:600
    int additionalOscPos = 0; // C:602

    // C:605-751 — bidirectional crossfade-point search (same algorithm as TimeStretcher.searchForCrossfadeOffset)
    boolean doSearch = lengthPerMovingAverage != 0
        && playHeads[TimeStretcher.PLAY_HEAD_OLDER].mode != LivePitchShifterPlayHead.PlayHeadMode.RAW_DIRECT; // C:605
    if (doSearch) {
      // C:610-618 — bounds checks
      doSearch = ((averagesStartPosNewHead - averagesStartOffsetFromHead) & (LiveInputBuffer.K_INPUT_RAW_BUFFER_SIZE - 1))
          < numRawSamplesProcessedLatest;
      if (doSearch) {
        doSearch = ((numRawSamplesProcessedLatest - averagesStartPosNewHead) & (LiveInputBuffer.K_INPUT_RAW_BUFFER_SIZE - 1))
    ...
    if (doSearch) {
      ...
      liveInputBuffer.getAveragesForCrossfade(newHeadTotals, averagesStartPosNewHead, lengthPerMovingAverage,
          numChannels); // C:621-622

      int bestDifferenceAbs = TimeStretcher.getTotalDifferenceAbs(oldHeadTotals, newHeadTotals); // C:624
      int timesSignFlipped = 0; // C:625
      int initialTotalChange = TimeStretcher.getTotalChange(oldHeadTotals, newHeadTotals); // C:627

      // Direction loop (C:629-749, gotos → loop structure matching TimeStretcher.searchForCrossfadeOffset)
      directionLoop:
      while (true) {
        ...
        for (int i = 0; i < TimeStretcher.K_NUM_MOVING_AVERAGES; i++) {
          ...
          readPos[i + 1] = (readPos[i] + lengthPerMovingAverage) & (LiveInputBuffer.K_INPUT_RAW_BUFFER_SIZE - 1);
        }
        ...
        if (searchDirection == -1) {
          if (numRawSamplesProcessedLatest < LiveInputBuffer.K_INPUT_RAW_BUFFER_SIZE) {
            ...
            if (searchSizeBoundary <= 0) {
              ...
              if (numFullDirectionsSearched < 2) { searchDirection = -searchDirection; continue directionLoop; }
              break directionLoop;
            }
          } else {
        ...
        do {
          for (int i = 0; i < TimeStretcher.K_NUM_MOVING_AVERAGES + 1; i++) {
            ...
            readPos[i] = (readPos[i] + searchDirection) & (LiveInputBuffer.K_INPUT_RAW_BUFFER_SIZE - 1);
          ...
          int differenceAbs = TimeStretcher.getTotalDifferenceAbs(oldHeadTotals, newHeadRunningTotals);
    ...
    if (phaseIncrement == K_MAX_SAMPLE_VALUE) { // C:778
      playHeads[TimeStretcher.PLAY_HEAD_NEWER].mode = LivePitchShifterPlayHead.PlayHeadMode.RAW_DIRECT; // C:779
      playHeads[TimeStretcher.PLAY_HEAD_NEWER].rawBufferReadPos =
          numRawSamplesProcessedAtNowTime & (LiveInputBuffer.K_INPUT_RAW_BUFFER_SIZE - 1); // C:780
    } else { // C:784
      playHeads[TimeStretcher.PLAY_HEAD_NEWER].mode = LivePitchShifterPlayHead.PlayHeadMode.RAW_REPITCHING; // C:785
      playHeads[TimeStretcher.PLAY_HEAD_NEWER].fillInterpolationBuffer(liveInputBuffer, numChannels); // C:789

...
public class SwingSynthConfigDialog extends JDialog {
  ...
  public SwingSynthConfigDialog(
    ...
    Runnable reloadDialog = () -> {
      dispose();
      new SwingSynthConfigDialog(owner, model, vm, bridge, trackIndex, projectModel).setVisible(true);
    };
  ...
  static int addSlider(JPanel panel, GridBagConstraints c, int row,
      ...
      ProjectModel projectModel, int trackIndex) {
    c.gridx = 0; c.gridy = row; c.gridwidth = 1;

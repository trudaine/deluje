...
public class SwingSynthConfigDialog extends JDialog {
  ...
  public SwingSynthConfigDialog(
    ...
    tabs.addTab("OSC", buildOscPanel(model, bridge, trackIndex));
...
// ── FM ──
  ...
  /**
   * Build the MOD FX tab: type dropdown, rate, depth, feedback, offset sliders.
   */
  private JPanel buildModFxPanel(SynthTrackModel model, BridgeContract bridge, int trackIndex) {
    JPanel panel = new JPanel(new GridBagLayout());
    panel.setBackground(new Color(0x22, 0x22, 0x22));
    GridBagConstraints c = new GridBagConstraints();
    c.fill = GridBagConstraints.HORIZONTAL;
    ...
    // ── Section header ──
    c.gridx = 0; c.gridy = row; c.gridwidth = 3;
    panel.add(sectionLabel("MODULATION FX"), c); row++;

    // Type dropdown
    c.gridx = 0; c.gridy = row; c.gridwidth = 1;
    panel.add(label("Type:"), c);
    c.gridx = 1; c.gridwidth = 2;
    JComboBox<String> typeCombo = new JComboBox<>(new String[]{"NONE", "CHORUS", "FLANGER", "PHASER"});
    ...
    typeCombo.addActionListener(ev -> {
      ...
      int ordinal = sel != null ? switch (sel) {
        case "NONE" -> 0; case "CHORUS" -> 1; case "FLANGER" -> 2; case "PHASER" -> 3;
        default -> 0;
      } : 0;
      model.setModFxType(sel);
      bridge.setModFxType(trackIndex, ordinal);
    });
    panel.add(typeCombo, c); row++;

    row = addSlider(panel, c, row, "Rate:",
        "Modulation FX rate (0-100%). Higher = faster modulation.",
        0, 100, (int)(bridge.getModFxRate(trackIndex) * 100),
        val -> { model.setModFxRate(val / 100f); bridge.setModFxRate(trackIndex, val / 100f); },
        "%", "modFxRate");

    row = addSlider(panel, c, row, "Depth:",
        "Modulation FX depth (0-100%). Higher = more pronounced effect.",
        0, 100, (int)(bridge.getModFxDepth(trackIndex) * 100),
        val -> { model.setModFxDepth(val / 100f); bridge.setModFxDepth(trackIndex, val / 100f); },
        "%", "modFxDepth");

    row = addSlider(panel, c, row, "Feedback:",
        "Modulation FX feedback (0-100%). Higher = more resonant, intense character.",
        0, 100, (int)(bridge.getModFxFeedback(trackIndex) * 100),
        val -> { model.setModFxFeedback(val / 100f); bridge.setModFxFeedback(trackIndex, val / 100f); },
        "%", "modFxFeedback");

    row = addSlider(panel, c, row, "Offset:",
        "Modulation FX offset (0-100%). Shifts the LFO phase for stereo variation.",
        0, 100, (int)(bridge.getModFxOffset(trackIndex) * 100),
        val -> { model.setModFxOffset(val / 100f); bridge.setModFxOffset(trackIndex, val / 100f); },
        "%", "modFxOffset");
  ...
  /**
   * Build the OSC tab: oscMix, noiseVol, portamento, osc1/2 sample-playback
   * (loop modes, reversed, time stretch, cents, interpolation, osc2Transpose, oscillatorSync).
   */
  private JPanel buildOscPanel(SynthTrackModel model, BridgeContract bridge, int trackIndex) {
    JPanel panel = new JPanel(new GridBagLayout());
    panel.setBackground(new Color(0x22, 0x22, 0x22));
    GridBagConstraints c = new GridBagConstraints();
    c.fill = GridBagConstraints.HORIZONTAL;
    ...
    // ── Mix ──
    c.gridx = 0; c.gridy = row; c.gridwidth = 3;
    panel.add(sectionLabel("MIX"), c); row++;

    row = addSlider(panel, c, row, "Osc Mix:",
        "Balance between oscillator 1 and 2 (0% = only osc1, 100% = only osc2)",
        0, 100, (int)(bridge.getOscMix(trackIndex) * 100),
        val -> { model.setOscMix(val / 100f); bridge.setOscMix(trackIndex, val / 100f); }, "%", "oscMix");

    row = addSlider(panel, c, row, "Noise Vol:",
        "Noise generator volume (0-100%). Adds white noise to the signal.",
        0, 100, (int)(bridge.getNoiseVol(trackIndex) * 100),
        val -> { model.setNoiseVol(val / 100f); bridge.setNoiseVol(trackIndex, val / 100f); }, "%", "noiseVol");

    row = addSlider(panel, c, row, "Portamento:",
        "Portamento / glide time (0-100%). Higher = slower pitch slides between notes.",
        0, 100, (int)(bridge.getPortamento(trackIndex) * 100),
        val -> { model.setPortamento(val / 100f); bridge.setPortamento(trackIndex, val / 100f); }, "%", "portamento");

    // ── Oscillator 1 Sample Playback ──
    c.gridx = 0; c.gridy = row; c.gridwidth = 3;
    panel.add(sectionLabel("OSCILLATOR 1 — SAMPLE PLAYBACK"), c); row++;

    // Loop mode
    c.gridx = 0; c.gridy = row; c.gridwidth = 1;
    panel.add(label("Loop Mode:"), c);
    c.gridx = 1; c.gridwidth = 2;
    ...
    osc1LoopCombo.setToolTipText("Sample playback loop mode: OFF = play once, LOOP = repeat, ONESHOT = play entire sample ignoring note-off");
    osc1LoopCombo.addActionListener(e -> {
      int idx = osc1LoopCombo.getSelectedIndex();
      model.setOsc1LoopMode(idx);
      bridge.setOsc1LoopMode(trackIndex, idx);
    });
    panel.add(osc1LoopCombo, c); row++;

    // Reversed
    c.gridx = 0; c.gridy = row; c.gridwidth = 1;
    panel.add(label("Reversed:"), c);
    c.gridx = 1; c.gridwidth = 2;
    ...
    osc1TsBox.addActionListener(e -> {
      boolean sel = osc1TsBox.isSelected();
      model.setOsc1TimeStretch(sel);
      bridge.setOsc1TimeStretch(trackIndex, sel ? 1 : 0);
    });
    panel.add(osc1TsBox, c); row++;

    // Time stretch amount
    row = addSlider(panel, c, row, "TS Amount:",
        "Time stretch amount (0-100%). Only relevant when time stretch is enabled.",
        0, 100, (int)(model.getOsc1TimeStretchAmount() * 100),
        val -> { model.setOsc1TimeStretchAmount(val / 100f); bridge.setOsc1TimeStretchAmount(trackIndex, val / 100f); },
        "%", "osc1TsAmt");

    // Cents
    row = addSlider(panel, c, row, "Cents:",
        "Fine pitch detune in cents (-50 to +50). 100 cents = 1 semitone.",
        -50, 50, model.getOsc1Cents(),
        val -> { model.setOsc1Cents(val); bridge.setOsc1Cents(trackIndex, val); }, "", "osc1Cents");

    // Linear interpolation
    c.gridx = 0; c.gridy = row; c.gridwidth = 1;
    panel.add(label("Interpolation:"), c);
    c.gridx = 1; c.gridwidth = 2;
    ...
    osc1LinBox.setToolTipText("Unchecked = zero-order hold (gritty); Checked = linear interpolation (smoother pitch shifting)");
    osc1LinBox.addActionListener(e -> {
    ...
    panel.add(osc1LinBox, c); row++;

    // ── Oscillator 2 Sample Playback ──
    c.gridx = 0; c.gridy = row; c.gridwidth = 3;
    panel.add(sectionLabel("OSCILLATOR 2 — SAMPLE PLAYBACK"), c); row++;
    ...
    osc2LoopCombo.setToolTipText("Sample playback loop mode: OFF = play once, LOOP = repeat, ONESHOT = play entire sample ignoring note-off");
    osc2LoopCombo.addActionListener(e -> {
      int idx = osc2LoopCombo.getSelectedIndex();
      model.setOsc2LoopMode(idx);
      bridge.setOsc2LoopMode(trackIndex, idx);
    });
    panel.add(osc2LoopCombo, c); row++;
    ...
    osc2TsBox.addActionListener(e -> {
      boolean sel = osc2TsBox.isSelected();
      model.setOsc2TimeStretch(sel);
      bridge.setOsc2TimeStretch(trackIndex, sel ? 1 : 0);
    });
    panel.add(osc2TsBox, c); row++;

    // Time stretch amount
    row = addSlider(panel, c, row, "TS Amount:",
        "Time stretch amount (0-100%). Only relevant when time stretch is enabled.",
        0, 100, (int)(model.getOsc2TimeStretchAmount() * 100),
        val -> { model.setOsc2TimeStretchAmount(val / 100f); bridge.setOsc2TimeStretchAmount(trackIndex, val / 100f); },
        "%", "osc2TsAmt");

    // Cents
    row = addSlider(panel, c, row, "Cents:",
        "Fine pitch detune in cents (-50 to +50). 100 cents = 1 semitone.",
        -50, 50, model.getOsc2Cents(),
        val -> { model.setOsc2Cents(val); bridge.setOsc2Cents(trackIndex, val); }, "", "osc2Cents");
    ...
    osc2LinBox.setToolTipText("Unchecked = zero-order hold (gritty); Checked = linear interpolation (smoother pitch shifting)");
    osc2LinBox.addActionListener(e -> {
    ...
    panel.add(osc2LinBox, c); row++;

    // Transpose
    row = addSlider(panel, c, row, "Transpose:",
        "Oscillator 2 transpose in semitones (-24 to +24).",
        -24, 24, model.getOsc2Transpose(),
        val -> { model.setOsc2Transpose(val); bridge.setOsc2Transpose(trackIndex, val); }, "st", "osc2Transpose");

    // ── Oscillator Sync ──
    c.gridx = 0; c.gridy = row; c.gridwidth = 3;
    panel.add(sectionLabel("OSCILLATOR SYNC"), c); row++;

    c.gridx = 0; c.gridy = row; c.gridwidth = 1;
    panel.add(label("Hard Sync:"), c);
    c.gridx = 1; c.gridwidth = 2;
    ...
    syncBox.setToolTipText("Oscillator hard sync: osc 2's phase is reset by osc 1's cycle, creating characteristic sync sweep sounds");
    syncBox.addActionListener(e -> {
      boolean sel = syncBox.isSelected();
      model.setOscillatorSync(sel);
      bridge.setOscillatorSync(trackIndex, sel ? 1 : 0);
    });
    panel.add(syncBox, c); row++;

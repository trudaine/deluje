...
public class AssetBrowserPopover extends Popup {
  ...
  public AssetBrowserPopover(BridgeContract bridge, int activeTrack, boolean kitsMode) {
    ...
    root.setStyle(
        "-fx-background-color: #1a1a1a; -fx-border-color: #00ff41; -fx-border-width: 2; -fx-border-radius: 5;");
    ...
    search.setStyle(
        "-fx-background-color: #333333; -fx-text-fill: white; -fx-font-family: 'Courier New';");

    ListView<AssetLibrary.AssetEntry> list = new ListView<>();
    list.setStyle(
        "-fx-base: #1a1a1a; -fx-control-inner-background: #1a1a1a; -fx-font-family: 'Courier New'; -fx-font-size: 11px;");
                  ...
                  if (entry.isFactory()) {
                    ...
                    if (kitsMode) {
                      org.chuck.deluge.model.KitTrackModel kit =
                          DelugeXmlParser.parseKit(is, entry.name());
                      // Implementation for loading kit path would go here
                      System.out.println(
                          "Loaded Factory Kit: " + kit.getName() + " -> " + kit.getSamplePath());
                    } else {
                      org.chuck.deluge.model.SynthTrackModel model =
                          DelugeXmlParser.parseSynth(is, entry.name());

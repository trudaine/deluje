...
public class SwingProjectSidebarPanel extends JPanel {
  ...
  private JComponent createLibraryTab() {
    javax.swing.tree.DefaultMutableTreeNode root = new javax.swing.tree.DefaultMutableTreeNode("SD CARD");
              ...
              try (java.io.InputStream is = getClass().getResourceAsStream(resourcePath)) {
                if (is != null) {
                  if ("KITS".equals(internalDir)) {
                    ...
                    for (int i = 0; i < 8; i++) {
                      if (i < sounds.size()) {
                        vm.setGlobalString("g_sample_" + (baseTrack + i), sounds.get(i).getSamplePath());
                      }
                    }
  ...
  private void addResourcesToTree(javax.swing.tree.DefaultMutableTreeNode root, String label, String internalDir) {
    javax.swing.tree.DefaultMutableTreeNode folder = new javax.swing.tree.DefaultMutableTreeNode(label);
    root.add(folder);
    ...
    try {
      ...
      if (url != null) {
        ...
        if (java.nio.file.Files.exists(path)) {
          try (java.util.stream.Stream<java.nio.file.Path> walk = java.nio.file.Files.walk(path, 1)) {

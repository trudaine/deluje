...
public class DelugeMainPanel extends BorderPane {
  ...
  public DelugeMainPanel(
    ...
    btnMonitor.setOnAction(
        e -> {
          javafx.scene.Node rightNode = getRight();
          if (monitorStage.isShowing()) {
            monitorStage.hide();
            if (rightNode == null)
              setRight(new javafx.scene.layout.StackPane()); // placeholder or restore!
          } else {
            setRight(null);
            if (rightNode != null) {
              javafx.scene.Scene s =
                  new javafx.scene.Scene((javafx.scene.Parent) rightNode, 280, 700);
    ...
    bottomBox.getChildren().addAll(globalParamPanel, masterFxPanel, statusPanel);

...
public class PianoRollView extends FirmwareView {
  ...
  public ActionResult selectButtonPress(boolean on) {
    if (on && clip.sound instanceof org.deluge.firmware.engine.FirmwareSound synth) {
      MatrixDriver.get()
          .pushUI(new MenuView(org.deluge.firmware.gui.menu.SoundEditor.createRootMenu(synth)));
  ...
  public ActionResult padAction(int x, int y, int velocity) {
    if (x < 16 && y < 8) {
      ...
      if (velocity > 0) {
        ...
        if (clip.sound != null) {
          if (clip.sound instanceof org.deluge.firmware.engine.FirmwareSound) {
            ((org.deluge.firmware.engine.FirmwareSound) clip.sound)

package org.deluge.firmware.gui.views;
...
public class KitView extends FirmwareView {
  ...
  public ActionResult selectButtonPress(boolean on) {
    if (on && clip.sound instanceof org.deluge.firmware.engine.FirmwareKit kit) {
      if (currentRow < kit.drumSounds.size()) {
        org.deluge.firmware.engine.FirmwareSound drum = kit.drumSounds.get(currentRow);
        MatrixDriver.get()
            .pushUI(new MenuView(org.deluge.firmware.gui.menu.SoundEditor.createRootMenu(drum)));
  ...
  public ActionResult padAction(int x, int y, int velocity) {
    if (x >= 0 && x < 18 && y >= 0 && y < 8) {
      if (velocity > 0) {
        ...
        if (clip.sound instanceof org.deluge.firmware.engine.FirmwareKit kit) {

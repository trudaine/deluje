...
public class PreferencesDialog extends JDialog {
  ...
  /**
   * Responsively sizes and centers the dialog within the usable screen work area. Clamps dimensions
   * so the dialog never exceeds maximum screen bounds (accounting for taskbars and display scaling
   * on small screens or high-DPI Raspberry Pi setups), and ensures the top title bar and bottom
  ...
  private JPanel buildSequencerPanel() {
    ...
    padRenderStyleCombo =
        new JComboBox<>(
            new String[] {
              "Silicone LED (Tone-Mapped Hardware Glow)", "Standard DAW (Flat Rectangular Grid)"
            });
    styleComboBox(padRenderStyleCombo);
    addField(
        panel,
        "Pad Rendering Style",
        padRenderStyleCombo,
        "Silicone backlit pads vs flat desktop DAW grid cells.",
        c,
        3);
  ...
  private void loadCurrentPreferences() {
    ...
    if (PreferencesManager.getPadRenderStyle() == PreferencesManager.PadRenderStyle.SILICONE_LED) {
      padRenderStyleCombo.setSelectedIndex(0);
    } else {
  ...
  private void saveBtnActionPerformed(java.awt.event.ActionEvent evt) {
    ...
    PreferencesManager.setPadRenderStyle(

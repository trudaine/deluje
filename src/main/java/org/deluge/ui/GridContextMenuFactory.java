...
/**
 * Factory class extracting context/popup menus from SwingGridPanel to reduce its size.
 * Uses package-private companion access to communicate with the panel.
 */
public class GridContextMenuFactory {
  ...
  public static void showSoloButtonContextMenu(SwingGridPanel panel, Component src, int x, int y, int trackIdx) {
    ...
    colorItem.addActionListener(
        e -> {
          Color chosen = javax.swing.JColorChooser.showDialog(panel, "Track Color", panel.trackColors[trackIdx]);
          if (chosen != null) {
  ...
  public static void showTrackContextMenu(SwingGridPanel panel, Component src, int x, int y, int trackIdx) {
    java.util.List<TrackModel> tracks = panel.projectModel.getTracks();
    if (trackIdx >= tracks.size()) return;
    TrackModel track = tracks.get(trackIdx);
    ...
    colorItem.addActionListener(
        e -> {
          Color chosen = JColorChooser.showDialog(panel, "Track Color", panel.trackColors[trackIdx]);
          if (chosen != null) {
            panel.trackColors[trackIdx] = chosen;
            track.setColourHex(
                "0x" + Integer.toHexString(chosen.getRGB() & 0xFFFFFF).toUpperCase());
        ...
        e -> {
          ...
          if (input != null) {
            try {
              ...
              int len = panel.bridge != null ? panel.bridge.getTrackLength(engineRow) : panel.stepCount;
  ...
  public static void showClipContextMenu(
    ...
    JRadioButtonMenuItem normalItem =
        new JRadioButtonMenuItem(
            "Normal", currentMode == ClipModel.PlayMode.NORMAL);
    ...
    JRadioButtonMenuItem loopItem =
        new JRadioButtonMenuItem(
            "Loop (green)", currentMode == ClipModel.PlayMode.LOOP);
    ...
    JRadioButtonMenuItem forwardItem =
        new JRadioButtonMenuItem(
            "Forward", currentDir == ClipModel.PlayDirection.FORWARD);
    ...
    JRadioButtonMenuItem reverseItem =
        new JRadioButtonMenuItem(
            "Reverse", currentDir == ClipModel.PlayDirection.REVERSE);
    ...
    JRadioButtonMenuItem pingPongItem =
        new JRadioButtonMenuItem(
            "Ping-Pong", currentDir == ClipModel.PlayDirection.PING_PONG);
    ...
    JRadioButtonMenuItem randomItem =
        new JRadioButtonMenuItem(
            "Random", currentDir == ClipModel.PlayDirection.RANDOM);
        ...
        evt -> {
          if (panel.bridge != null) {
            java.util.List<TrackModel> tracks =
                panel.projectModel != null ? panel.projectModel.getTracks() : java.util.Collections.emptyList();
            if (panel.viewMode == SwingGridPanel.GridViewMode.CLIP || panel.viewMode == SwingGridPanel.GridViewMode.AUTOMATION) {
              for (int i = 0; i < tracks.size(); i++) {

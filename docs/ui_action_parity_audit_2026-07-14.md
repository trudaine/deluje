...
| **`TEMPO_ENC` (Rotation)** | Fine (`+/- 1.0 BPM`), Pushed (`+/- 5.0 BPM`), Shift (`Swing %`) (`lines 966-980`) | `playbackHandler.commandEditTempoFine/Coarse/Swing()` (`playback_handler.cpp:2082, 2104`) | **1:1 Parity Verified (Fixed Today).** Corrected fine vs coarse push ordering. |

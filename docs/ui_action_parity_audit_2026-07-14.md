...
| **`LOAD` / `SAVE`** | Calls `listener.onLoadProject()` / `listener.onSaveProject()` (`lines 1307-1308`) | `View::buttonAction(SAVE/LOAD)` (`view.cpp:227-326`) | **CORRECTED (2026-07-14) — original claim was BACKWARDS.** Re-read the actual C: a **plain** LOAD press (no Shift) opens `loadSongUI` (`view.cpp:295-300`); **Shift+LOAD** opens a **Clear Song confirmation** (`context_menu::clearSong`, `view.cpp:277-292`) — not a load dialog at all. **Shift+SAVE does nothing** (`view.cpp:234`'s `!Buttons::isShiftButtonPressed()` guard means the save-button-holding mode is never entered while Shift is held); a plain SAVE press opens `saveSongUI`. **Re-derived and verified (2026-07-14):** the "instrument preset" save/load behavior the original row described is NOT a SYNTH/KIT/MIDI/CV+SAVE/LOAD chord on this button (that earlier guess was itself wrong). Two distinct, unrelated real mechanisms exist: (1) `SoundEditor::buttonAction(SAVE)` (`sound_editor.cpp:447-461`) — pressing SAVE **while the per-instrument Sound Editor menu is open** (`isUIInstrumentClipView`, no Shift) opens `saveInstrumentPresetUI`; nothing to do with SYNTH/KIT/MIDI/CV. (2) `ArrangerView::buttonAction(SYNTH/KIT)` (`arranger_view.cpp:343-383`) — in the **Arranger view**, while holding an arrangement row to audition it (`UI_MODE_HOLDING_ARRANGEMENT_ROW_AUDITION`), pressing SYNTH or KIT *with LOAD also held* opens `loadInstrumentPresetUI` scoped to that row (MIDI/CV skip straight to a type change instead); there is no SAVE-side equivalent. Java's current plain LOAD/SAVE already broadly matches the plain-press behavior in spirit (opens a file chooser instead of the exact hardware UI). **Do not implement the original proposal** (item 3 in §5 below) — it would make Shift+LOAD open a preset browser where hardware opens a destructive-sounding confirmation dialog. |
...
| **`AFFECT_ENTIRE` / `TRIPLETS` / `CROSS_SCREEN` / `SYNC_SCALING` / `LEARN` / `TAP_TEMPO`** | Toggles state booleans + updates OLED (`lines 1321-1369`) | `View::buttonAction(...)` (`view.cpp:330` for `SYNC_SCALING`) | **1:1 Parity Verified.** |
| **`SCALE_MODE`** | **Fixed (2026-07-14):** press only arms a short-press timer (`SwingHardwareTopPanel.mousePressed`); the toggle itself fires on `mouseReleased` only if the press was short (`SCALE_MODE_SHORT_PRESS_MS=100`, mirroring `FlashStorage::holdTime`'s default). Guarded by 3 new tests in `SwingHardwareTopPanelTest` (no toggle on press, toggle on short release, no toggle on long-held release). | `InstrumentClipView::handleScaleButtonAction` (`instrument_clip_view.cpp:855-916`) | **Fixed.** Previously toggled instantly on mouse-down; C only toggles on button **release**, gated by `isShortPress(scaleButtonPressTime)` (`instrument_clip_view.cpp:906`, `FlashStorage::holdTime` default 100ms, `flash_storage.cpp:348-349`). The hold+note-pad root-picker combo (`oneNoteAuditioning()` branch, `instrument_clip_view.cpp:882-887`) and the root-note LED flash (`commandFlashRootNote`) are real hardware behavior but out of scope for this pass — not modeled. |
...
1. ~~**Wire Gold Knob Push Clicks...**~~ **Both halves FIXED (2026-07-14).** Re-derived the full
   ...
   both against the live running app via direct event dispatch, not just unit tests. **Persistence
   fixed (2026-07-14):** added the `<modKnobs><modKnob cc="..."/>` write/read pair to
   `ProjectSerializer`/`SongXmlParser`, matching `MIDIInstrument::writeDataToFile`'s exact
   "none"/"bend"/"aftertouch" sentinel convention (`midi_instrument.cpp:298-334`,
   `definitions_cxx.hpp:730-739`); guarded by a round-trip test
   (`ProjectSerializerTest#testMidiTrackModKnobCcAssignmentsSurviveRoundtrip`). CC assignments now
   survive save/reload.
...
3. ~~**Add Shift + `SAVE` / `LOAD` Sub-Menu Shortcuts**~~ **RETRACTED — the original claim was
   ...
   The real "instrument preset" save/load is two distinct, verified mechanisms, neither a
   SYNTH/KIT/MIDI/CV+SAVE/LOAD chord: SAVE-while-Sound-Editor-open (`sound_editor.cpp:447-461`,
   opens `saveInstrumentPresetUI`), and Arranger-row-hold + SYNTH/KIT + LOAD-held
   (`arranger_view.cpp:343-383`, opens `loadInstrumentPresetUI`). The Arranger one needs a new
   arrangement-row-audition-hold state machine the Java arranger view doesn't have at all today
   (`ArrangerGridPanel`/`ArrangerTimelineController`) — a genuinely large lift, not a small combo
   wiring — deferred (2026-07-14) in favor of the 133/144/137 residual probe.

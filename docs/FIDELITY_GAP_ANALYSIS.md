...
2. **Audit of Remaining Sub-0.70 Scorers (`100 Noise Lead`, `121 Tiny Lights`, `149 Cold 5th Pad`, `133 80s Strings`)**: Audited the signal and modulation paths for the last remaining lower scorers in the time-resolved spectral scorecard. Confirmed that all four presets feature unsynced, free-running LFOs (`lfo1` with `syncLevel=0`, or `lfo2`) patched directly to sensitive timbral parameters (`133`: `lfo1->pitch`; `100`: `lfo2->lpfFrequency`; `121`: `lfo1->lpfResonance`; `149`: `lfo1->pitch` and `lfo2->lpfResonance`). Because the manual reference recordings were captured after arbitrary LFO phase drift on running hardware while the Java test harness initiates rendering at phase 0, time-resolved short-time Fourier transform (STFT) cosine similarity reflects expected modulation phase variance rather than a subtractive or filter DSP discrepancy.
...
4. **Conclusion**: With the subtractive core, drive/saturation primitive stack, clip-param semantics, and filter modes verified bit-faithful and all documented DSP gaps closed or root-caused to measurement methodology (unsynced LFO drift / near-silent slices), the core synth engine reaches verified DSP parity with C++ firmware.
